...
func (f *helloFilter) Response(ctx filters.FilterContext) {
	...
	chunk_reg := regexp.MustCompile(".*chunk.*\\.js")

	if chunk_reg.MatchString(request_url.Path) {
		bodyRes, err := readRes(ctx)
		if err != nil {
			fmt.Printf("读取错误%s", err)
		}

		replace_str := replaceHost(bodyRes)
...
func replaceDomain(res []byte) []byte {
	rep_host := Config.Domain + ":" + Config.Port
	reg := regexp.MustCompile("ai.12348.gov.cn")
	replace_str := reg.ReplaceAll(res, []byte(rep_host))

	reg = regexp.MustCompile("hrpay.laway.cn")
	replace_str = reg.ReplaceAll(replace_str, []byte(rep_host+"/hrpay"))

	reg = regexp.MustCompile("newsystem.laway.cn")
	replace_str = reg.ReplaceAll(replace_str, []byte(rep_host+"/newsystem"))

	return replace_str
}

func replaceHost(res []byte) []byte {
	rep_host := "\"https://ai.12348.gov.cn\""
	reg := regexp.MustCompile("window.location.origin")
	replace_str := reg.ReplaceAll(res, []byte(rep_host))
	return replace_str
}
